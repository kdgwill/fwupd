					       ...
					       GError **error)
{
	...
	for (guint retries = 0; ; retries++) {
		...
		if (!fu_synaptics_rmi_ps2_device_read_byte (self, buf, 1000, &error_local)) {
			if (retries++ > 2) {
				g_propagate_prefixed_error (error,
							    g_steal_pointer (&error_local),
						      ...
						      GError **error)
{
	...
	for (guint i = 0; i < req_sz; ++i) {
		guint8 tmp = 0;
		if (!fu_synaptics_rmi_ps2_device_read_byte (self, &tmp, 1000, error)) {
...
fu_synaptics_rmi_ps2_device_init (FuSynapticsRmiPs2Device *self)
{
	...
	fu_synaptics_rmi_device_set_kind (FU_SYNAPTICS_RMI_DEVICE (self),
					  RMI_DEVICE_KIND_PS2);
}
