...
static const gchar *
fu_synaptics_rmi_device_kind_to_string (RmiDeviceKind kind)
{
	if (kind == RMI_DEVICE_KIND_HID)
		return "hid";
	if (kind == RMI_DEVICE_KIND_PS2)
		return "ps2";
	return NULL;
}
...
fu_synaptics_rmi_device_to_string (FuUdevDevice *device, guint idt, GString *str)
{
	...
	fu_common_string_append_kv (str, idt, "Kind",
				    fu_synaptics_rmi_device_kind_to_string (priv->kind));
					  ...
					  GError **error)
{
	...
	size_expected = ((gsize) priv->flash.block_count_fw * (gsize) priv->flash.block_size) +
			fu_synaptics_rmi_firmware_get_signature_size (FU_SYNAPTICS_RMI_FIRMWARE (firmware));
...
gboolean
fu_synaptics_rmi_device_enter_backdoor (FuSynapticsRmiDevice *self, GError **error)
{
	FuSynapticsRmiDeviceClass *klass_rmi = FU_SYNAPTICS_RMI_DEVICE_GET_CLASS (self);
				       ...
				       GError **error)
{
	...
	g_debug ("f34: %d", priv->f34->function_version);
	g_debug ("f34_status: %d", f34_status);
	g_debug ("f34_command: %d", f34_command);
	g_debug ("f34_enabled: %d", f34_enabled);

	/* PS/2 */
	if (fu_synaptics_rmi_device_get_kind (self) == RMI_DEVICE_KIND_PS2) {
		if (f34_command == 0) {
...
RmiDeviceKind
fu_synaptics_rmi_device_get_kind (FuSynapticsRmiDevice *self)
{
	FuSynapticsRmiDevicePrivate *priv = GET_PRIVATE (self);
	return priv->kind;
}
...
void
fu_synaptics_rmi_device_set_kind (FuSynapticsRmiDevice *self, RmiDeviceKind kind)
{
	FuSynapticsRmiDevicePrivate *priv = GET_PRIVATE (self);
	priv->kind = kind;
}
